...
export default function App() {
      ...
      if (splitResults.length > 0) {
        splitResults.forEach(res => {
          ...
          nextAppliedGroups.push({
            ...
            decorations: [{ id: `dec_${Date.now()}_0`, type: 'megane', cx, cy, angle: angleDeg, scale: Math.max(0.1, 1.0 + (symbolScale - 1.0) * 2.0) * 2.16 }]
          });
        });
  ...
  const handleRegionLabelMouseDown = useCallback((e, regionKey, dragMode, center, initialScale = 1.0) => {
    const pt = getSvgPoint(e);
    if (!pt) return;
    const prev = currentRegionOverrides[regionKey] || { dx: 0, dy: 0 };
    setActiveDeco({ type: 'region_label', id: regionKey, dragMode, startCx: center.x, startCy: center.y, startScale: initialScale, startMouseX: pt.x, startMouseY: pt.y, startDx: prev.dx, startDy: prev.dy });
  ...
  const handleChibanLabelMouseDown = useCallback((e, polyId, dragMode, center, initialScale = 1.0) => {
    const pt = getSvgPoint(e);
    if (!pt) return;
    const prev = currentChibanOverrides[polyId] || { dx: 0, dy: 0 };
    setActiveDeco({ type: 'chiban_label', id: polyId, dragMode, startCx: center.x, startCy: center.y, startScale: initialScale, startMouseX: pt.x, startMouseY: pt.y, startDx: prev.dx, startDy: prev.dy });
    ...
    if (activeDeco) {
      if (activeDeco.type === 'region_label') {
        ...
        if (activeDeco.dragMode === 'move') {
          const newDx = (activeDeco.startDx || 0) + (pt.x - activeDeco.startMouseX);
          const newDy = (activeDeco.startDy || 0) + (pt.y - activeDeco.startMouseY);
          setDragRegionOverride({ ...prev, regionKey: activeDeco.id, dx: newDx, dy: newDy });
        ...
        if (activeDeco.dragMode === 'move') {
          const newDx = (activeDeco.startDx || 0) + (pt.x - activeDeco.startMouseX);
          const newDy = (activeDeco.startDy || 0) + (pt.y - activeDeco.startMouseY);
          setDragChibanOverride({ ...prev, polyId: activeDeco.id, dx: newDx, dy: newDy });
        } else if (activeDeco.dragMode === 'scale') {
          const startDx = activeDeco.startMouseX - activeDeco.startCx;
          const startDy = activeDeco.startMouseY - activeDeco.startCy;
          const startDist = Math.sqrt(startDx * startDx + startDy * startDy);
          ...
          <div className="absolute inset-0 w-full h-full select-none">
            <svg ref={svgRef} className={`w-full h-full outline-none touch-none bg-neutral-100 overflow-visible select-none ${mode === 'add_text' || mode === 'draw' ? 'cursor-crosshair' : ''}`} viewBox={`${viewBox.x} ${viewBox.y} ${viewBox.w} ${viewBox.h}`} 
              ...
              <RegionLabelLayer 
                ...
                dragRegionOverride={dragRegionOverride} 
                labelFontSize={labelFontSize} 
                selectedDeco={selectedDeco} 
                mode={mode} 
                viewBox={viewBox} 
                handleRegionLabelMouseDown={handleRegionLabelMouseDown} 
              />
              <PolygonLayer 
                ...
                showLabels={showLabels} 
                dragChibanOverride={dragChibanOverride} 
                currentChibanOverrides={currentChibanOverrides} 
                viewBox={viewBox} 
                ...
                handleChibanLabelMouseDown={handleChibanLabelMouseDown} 
              />
                      ...
                      <div key={idx} className="flex items-center justify-between bg-neutral-50 hover:bg-neutral-100 p-2 rounded border border-neutral-200 transition-colors">
                        <div className="flex flex-col overflow-hidden mr-2">
                          ...
                          <div className="flex gap-1 mr-2">
                            <button onClick={() => toggleFlipBg(idx, 'x')} className="px-2 py-1 bg-white border border-neutral-300 text-neutral-600 hover:bg-neutral-100 rounded text-[10px] shadow-sm font-medium" title="左右反転">左右反転</button>
                            <button onClick={() => toggleFlipBg(idx, 'y')} className="px-2 py-1 bg-white border border-neutral-300 text-neutral-600 hover:bg-neutral-100 rounded text-[10px] shadow-sm font-medium" title="上下反転">上下反転</button>
                          </div>
                        <button 
                          onClick={() => {
