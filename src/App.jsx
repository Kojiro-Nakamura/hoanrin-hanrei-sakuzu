...
export default function App() {
  ...
  useEffect(() => {
    if (error && error.includes("蠅・阜邱壹′隕九▽縺九ｊ縺ｾ縺帙ｓ縺ｧ縺励◆")) {
      setError(null);
    }
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl border border-neutral-200 p-6 max-w-sm w-full flex flex-col gap-4">
            <h3 className="text-lg font-bold text-neutral-800 flex items-center gap-2"><Home className="w-5 h-5 text-indigo-600"/>蜈ｨ縺ｦ縺ｮ繝ｪ繧ｻ繝・ヨ</h3>
            <p className="text-sm text-neutral-600">迴ｾ蝨ｨ縺ｮ菴懈･ｭ繝・・繧ｿ縺ｯ蜈ｨ縺ｦ遐ｴ譽・・繝ｪ繧ｻ繝・ヨ縺輔ｌ縺ｾ縺吶ゅｈ繧阪＠縺・〒縺吶°・・/p>
            <div className="flex gap-2 justify-end mt-2">
              <button onClick={() => setShowResetConfirm(false)} className="px-4 py-2 rounded-lg text-sm font-bold text-neutral-600 bg-neutral-100 hover:bg-neutral-200">繧ｭ繝｣繝ｳ繧ｻ繝ｫ</button>
              <button onClick={confirmReset} className="px-4 py-2 rounded-lg text-sm font-bold text-white bg-red-600 hover:bg-red-700">繝ｪ繧ｻ繝・ヨ</button>
            </div>
          </div>
        </div>
      )}
      ...
      <main className="flex-1 relative overflow-hidden">
        {loading && (
          <div className="absolute inset-0 bg-white/50 backdrop-blur-sm flex flex-col items-center justify-center z-50">
            ...
            <p className="text-lg font-bold text-neutral-800">繝・・繧ｿ繧定ｪｭ縺ｿ霎ｼ縺ｿ荳ｭ...</p>
          </div>
        )}
        ...
        {error && (
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-red-50 text-red-600 px-6 py-3 rounded-lg shadow-lg border border-red-200 flex items-center gap-3 z-50 animate-in fade-in slide-in-from-top-4">
            ...
            <button onClick={() => setError(null)} className="ml-4 hover:bg-red-100 p-1 rounded-md transition-colors">ﾃ・/button>
          </div>
        )}
          ...
          <div className="absolute inset-0 w-full h-full select-none">
            ...
            {mode !== 'select' && mode !== 'edit_deco' && (
              <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-white/90 border border-indigo-200 text-indigo-700 px-4 py-1.5 rounded-full shadow z-20 font-bold text-[10px] sm:text-xs flex items-center gap-1.5 pointer-events-none backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4 whitespace-nowrap">
                ...
                <span>繧ｯ繝ｪ繝・け: 霑ｽ蜉 / Enter,繝繝悶Ν繧ｯ繝ｪ繝・け: 螳御ｺ・/ 蜿ｳ繧ｯ繝ｪ繝・け: 謌ｻ繧・/ ESC: 蜿匁ｶ・/span>
              </div>
            )}

            <div className="absolute top-4 left-4 flex flex-col gap-2 pointer-events-auto z-10">
              <div className="flex items-center gap-2">
                <button onClick={() => fitToBoundingBox(data.boundingBox)} className="p-2.5 rounded-lg shadow-md hover:bg-neutral-50 text-neutral-700 transition-colors border border-neutral-200 bg-white" title="蜈ｨ菴薙ｒ陦ｨ遉ｺ"><Maximize className="w-5 h-5" /></button>
                <button onClick={() => setShowLabels(!showLabels)} className={`p-2.5 rounded-lg shadow-md transition-colors border border-neutral-200 ${showLabels ? 'bg-indigo-100 text-indigo-700' : 'bg-white text-neutral-700'}`} title="蝨ｰ逡ｪ縺ｮ陦ｨ遉ｺ蛻・崛"><Hash className="w-5 h-5" /></button>
                <button onClick={() => setShowMap(!showMap)} disabled={!data.coordinateSystem} className={`p-2.5 rounded-lg shadow-md transition-colors border border-neutral-200 ${showMap ? 'bg-indigo-100 text-indigo-700' : 'bg-white text-neutral-700'} ${!data.coordinateSystem && 'opacity-50'}`} title="蝨ｰ逅・劼蝨ｰ蝗ｳ"><Globe className="w-5 h-5" /></button>
                {showMap && (
                  <div className="bg-white px-3 py-2 rounded-lg shadow-md border border-neutral-200 flex items-center gap-2">
                    ...
                    <select value={mapType} onChange={e => setMapType(e.target.value)} className="bg-transparent text-sm font-medium text-neutral-700 outline-none cursor-pointer">
                      <option value="pale">豺｡濶ｲ蝨ｰ蝗ｳ・郁ｦ九ｄ縺吶＞・・/option>
                      <option value="std">讓呎ｺ門慍蝗ｳ</option>
                      <option value="seamlessphoto">闊ｪ遨ｺ蜀咏悄</option>
                    </select>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleUndo} disabled={historyIndex <= 0} className={`p-2.5 rounded-lg shadow-md transition-colors border border-neutral-200 bg-white ${historyIndex <= 0 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-neutral-50 text-neutral-700'}`} title="蜈・↓謌ｻ縺・(Ctrl+Z)"><Undo className="w-5 h-5" /></button>
                <button onClick={handleRedo} disabled={historyIndex >= history.length - 1} className={`p-2.5 rounded-lg shadow-md transition-colors border border-neutral-200 bg-white ${historyIndex >= history.length - 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-neutral-50 text-neutral-700'}`} title="繧・ｊ逶ｴ縺・(Ctrl+Y)"><Redo className="w-5 h-5" /></button>
              </div>
            </div>
            ...
            <div className="absolute bottom-6 right-6 flex flex-col items-end gap-1 pointer-events-none z-10">
              {showMap && <div className="bg-white/80 backdrop-blur px-2 py-1 rounded shadow-sm text-[10px] text-neutral-600 pointer-events-auto"><a href="https://maps.gsi.go.jp/development/ichiran.html" target="_blank" rel="noreferrer" className="hover:underline">蜃ｺ蜈ｸ・壼嵜蝨溷慍逅・劼</a></div>}
            </div>
          </div>
        )}
      </main>
