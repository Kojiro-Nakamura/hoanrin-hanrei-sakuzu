...
export const Header = ({ fileInfo, coordinateSystem, onReset, onExportDXF, onLoadFile }) => (
  <header className="flex items-center justify-between px-6 py-3 bg-white border-b border-neutral-200 shadow-sm z-10 shrink-0">
    ...
    {fileInfo && (
      <div className="flex items-center gap-4">
        ...
        <button onClick={onExportDXF} className="ml-2 flex items-center gap-1.5 px-3 py-1.5 bg-white text-indigo-700 hover:bg-indigo-50 border border-indigo-200 rounded-md transition-colors text-sm font-bold shadow-sm" title="DXFファイルとして保存">
          <Download className="w-4 h-4" /> DXF保存
        </button>
        <label className="flex items-center gap-1.5 px-3 py-1.5 bg-white text-indigo-700 hover:bg-indigo-50 border border-indigo-200 rounded-md transition-colors text-sm font-bold shadow-sm cursor-pointer" title="XML/KML追加読込">
          <UploadCloud className="w-4 h-4" /> 追加読込
          <input type="file" multiple className="hidden" accept=".xml,.kml" onChange={e => { Array.from(e.target.files).forEach(f => onLoadFile(f, true)); e.target.value = ''; }} />
        </label>
